...
class NetQueue {
  String _logTag = (NetQueue).toString();
  Logger _logger;
  ...
  void start() {
    _logger.debug(_logTag, "start");
    _store.onChange.listen((event) {
      if (event.queue.tick != _stamp) {
        _stamp = event.queue.tick;
        _logger.debug(_logTag, "queue changed, try to process");
    ...
    if (_store.state.queue.now != null || _store.state.queue.list.length > 0) {
      _logger.debug(_logTag, "queue is not empty, start to process");
  ...
  void addQueueItem(
    ...
    _logger.info(_logTag, "addQueueItem $type $action");
  ...
  void addBeforeQueueItem(
    ...
    _logger.info(_logTag, "addBeforeQueueItem $type $action");
  ...
  void registerQueueItemAction(
    ...
    _logger.info(_logTag, "registerQueueItemAction $type $act");
  ...
  Future<bool> _executeQueueItem(Store<AppState> store, QueueItem item) async {
    _logger.info(_logTag, "_executeQueueItem ${item.type} ${item.action}");
  ...
  void _queueLoop() {
    ...
    if (q.now != null) {
      _logger.debug(_logTag, "process current one");
      // process current one
      _executeQueueItem(_store, q.now).then((bool succ) {
        if (succ) {
          _logger.debug(_logTag, "process current one succ");
          // if succ, remove first one and run again
          _store.dispatch(DoneQueueItemAction());
        } else {
          _logger.debug(_logTag, "process current one failed");
      ...
      if (q.list.length > 0) {
        _logger.debug(_logTag, "move top in queue to current");
        _store.dispatch(ProcessQueueItemAction());
      } else {
        _logger.debug(_logTag, "queue is empty");
        _sleep();
      }
      _status = NetQueueStatus.IDLE;
    }
